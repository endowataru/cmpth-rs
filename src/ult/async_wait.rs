...
use crate::ult::system::AsyncWorkerSystem;
...
pub struct SuspendedFuture<S: AsyncWorkerSystem> {
    waker: AtomicPtr<Waker>,
    _marker: PhantomData<S>,
}

unsafe impl<S: AsyncWorkerSystem> Send for SuspendedFuture<S> {}
unsafe impl<S: AsyncWorkerSystem> Sync for SuspendedFuture<S> {}

impl<S: AsyncWorkerSystem> Default for SuspendedFuture<S> {
    fn default() -> Self {
        SuspendedFuture { waker: AtomicPtr::new(ptr::null_mut()), _marker: PhantomData }
    }
}

impl<S: AsyncWorkerSystem> Resumable<S> for SuspendedFuture<S> {
    fn is_set(&self) -> bool {
