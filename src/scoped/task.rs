//! [`TaskRef`]/[`StackTask`]/[`Latch`] — a stealable reference to a task living
...
//!
//! [`super::async_engine`] reuses [`TaskRef`] too (its trampoline just drives
//! a `Future` to completion instead of calling a `FnOnce` once), but needs
//! its own task/latch shape that can hold a registered [`Waker`](std::task::Waker) —
...
/// Type-erased, two-word reference to a task living on someone's native
...
impl TaskRef {
    /// Build a `TaskRef` from raw parts — used by [`super::async_engine`],
    /// whose tasks are `Arc`-owned rather than a `StackTask` this module
...
/// can read `func`/`result` through `&StackTask` — synchronized entirely by
...
pub(super) struct StackTask<F, R> {
    pub(super) latch: Latch,
    func: UnsafeCell<Option<F>>,
    result: UnsafeCell<Option<R>>,
}

unsafe impl<F: Send, R: Send> Sync for StackTask<F, R> {}

impl<F, R> StackTask<F, R>
where
    F: FnOnce() -> R + Send,
    R: Send,
{
    pub(super) fn new(f: F) -> Self {
        StackTask { latch: Latch::new(), func: UnsafeCell::new(Some(f)), result: UnsafeCell::new(None) }
    }

    pub(super) fn as_task_ref(&self) -> TaskRef {
        TaskRef { data: self as *const Self as *const (), execute_fn: Self::execute_trampoline }
    }

    unsafe fn execute_trampoline(this: *const ()) {
        let this = unsafe { &*(this as *const Self) };
        let f = unsafe { (*this.func.get()).take() }.expect("cmpth: StackTask executed twice");
        let r = f();
    ...
    /// Run inline: we (the pusher) popped this task back off ourselves,
    /// unstolen. No latch involved — nobody else could have touched it.
    pub(super) fn run_inline(&self) -> R {
        let f = unsafe { (*self.func.get()).take() }.expect("cmpth: StackTask run twice");
        f()
    }

    pub(super) fn take_result(&self) -> R {
        unsafe { (*self.result.get()).take() }.expect("cmpth: StackTask latch set without a result")
    }
}
